...
app.get("/challenge/:id", async (c) => {
	...
	const challengeId = parseInt(idParam, 10);
  ...
  `;
	...
	const challenge = await c.env.DB.prepare(sqlQuery).bind(challengeId).first();
...
app.post("/challenge/guess", async (c) => {
  ...
  `;
	...
	const challenge = await c.env.DB.prepare(sqlQuery).bind(token).first();
  ...
  `;
	...
	if (!upd.meta || (upd.meta.changes ?? 0) !== 1) {
		...
		return c.json(
			{
				error:
					"Write conflict; please retry your last guess with the latest token.",
			},
			409,
		);
	}
